...
class UserRemoteDataSource {
  ...
  Future<void> upsertUser(UserModel model) async {
    await db.collection('users').doc(model.uid).set(model.toJson(), SetOptions(merge: true));
  }

  Future<void> updateActiveContext(String uid, ActiveContextModel ctx) async {
    await db.collection('users').doc(uid).set({'activeContext': ctx.toJson(), 'updatedAt': FieldValue.serverTimestamp()}, SetOptions(merge: true));
  }

  Future<List<MembershipModel>> memberships(String uid) async {
    final snap = await db.collection('memberships').where('userId', isEqualTo: uid).get();
    return snap.docs.map((d) => MembershipModel.fromJson(d.data())).toList();
  }
}
